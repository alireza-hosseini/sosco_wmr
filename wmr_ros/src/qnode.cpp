...
bool QNode::init() {
    ...
    QCoreApplication coreApplication(init_argc,init_argv);
    wmr = new WMR(&coreApplication);
    ...
    frame_id_ir_ = tf::resolve(tf_prefix_,"ir_link");
...
void QNode::run() {
    ...
    while ( ros::ok() ) {
        wmrRobot->setLinearVel(cmd_vel_.linear.x);
        wmrRobot->setRotVel(cmd_vel_.angular.z);
        wmrRobot->readStatus();
        wmrRobot->readSharp();
        publishOdometry();
        publishIRProximity();
...
void QNode::cmdVelCallback(const geometry_msgs::Twist::ConstPtr& cmd_vel_msg) {
    ROS_INFO_ONCE("First velocity command received...");
...
void QNode::publishIRProximity() {
    ...
    pointCloud.header.frame_id = frame_id_ir_;
    for(int i=0; i<IR_SENSORS_NUM; i++) {
        WmrIRReading reading = wmrRobot->getIRProximity(i);
    ...
    ir_proximity_sensors_pub_.publish(pointCloud);
}
